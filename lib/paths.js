...
var paths = {
  ...
  remove: function(path, options, done) {
    ...
    var end = function(err, deleted) {
     debug('Paths deleted: ', deleted)
     return done(err, deleted)
    }
      ...
      if(isArray(path)) {
        async.each(path, function(item, cb) {
          ...
          if(is_watched[path] === true) {
            ...
            deleted_paths.push(item)
            Paths.findByIdAndRemove(item, cb)
          }
          ...
          return end(null, deleted_paths)
        })

      } else if(isObjectId(path)) {
        Paths.findByIdAndRemove(path, function(err) {
          if(err === null) {
            deleted_paths.push(path)
          }

          end(err, deleted_paths)
        })
      } else {
        Paths.findOneAndRemove({path : path}, function(err) {
          if(err === null) {
            deleted_paths.push(path)
          }
